...
async function main() {
  ...
  if (fixtures.length === 104) ok("104 fixtures present.");
  else if (fixtures.length > 0) fail(`Expected 104 fixtures, got ${fixtures.length}. Investigate before proceeding.`);
  else fail("No fixtures returned. STOP and report.");
